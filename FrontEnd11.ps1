...
$RoomNumber = 1
$syncHash.Rooms = @()
foreach ($EQ3Thermostat in (Read-FromPostgreSQL -DBServer $DBServer -DBName $DBName -DBUser $DBUser -DBPassword $DBPassword -Query 'select * from eq3thermostats ORDER BY eq3id')) 
{ 
...
do
{
    ...
    if ($SystemOnline)
    {
        if ($syncHash.RoomButtonClicked -ne $false)
        {
            ...
            $syncHash.CurrentRoomLabel = ((Read-FromPostgreSQL -DBServer $DBServer -DBName $DBName -DBUser $DBUser -DBPassword $DBPassword -Query "select * from eq3thermostats where eq3macaddress = '$TempMACAddress'")).eq3macaddress 
            if ((((Read-FromPostgreSQL -DBServer $DBServer -DBName $DBName -DBUser $DBUser -DBPassword $DBPassword -Query "select * from eq3thermostats where eq3macaddress = '$TempMACAddress'")).friendlyname) -match '^[0-9a-zA-Z]') { $syncHash.CurrentRoomLabel = (($syncHash.Rooms | Where-Object {$_.RoomNumber -eq $RoomSelected}).FriendlyName) }
            $syncHash.Window.Dispatcher.invoke("Normal", [action][scriptblock]::create({
        ...
        if ($syncHash.RoomSelectionKeyPressedDate)
        {
            if ((Get-Date) -gt (($syncHash.RoomSelectionKeyPressedDate).AddSeconds(1.5)))
            {
                ...
                Set-EQ3FriendlyName -DBServer $DBServer -DBName $DBName -DBUser $DBUser -DBPassword $DBPassword -EQ3MACAddress $TempMACAddress -FriendlyName $syncHash.RoomSelectionText
            }
        }
